...
class RecipeCard extends Component {
  ...
  clickHandler(event) {
    ...
    const url = `https://api.spoonacular.com/recipes/${this.props.recipe.id}/information/?apiKey=${process.env.REACT_APP_API}&includeNutrition=false`;

    fetch(url)
      .then((response) => response.json())
      .then((data) => this.UrlHandler(data));
  }
      ...
      <>
        <div className='card border-0 text-start bg-transparent col-xs-12 col-sm-6 col-md-4 '>
          <img
            ...
            src={`https://spoonacular.com/recipeImages/${this.props.recipe.id}-636x393.jpg`}
          />
